...
Picked up the test-rewrite continuation and found freebuff's §152 rewrite mid-flight on the same files — stood down per the coordination protocol, monitored to quiet, then verified the final state: gate fully green on the current tree (lint 0 · TS 0 · **330/330** · build 0 · `acc check` 0 errors, only the documented ACC014 + pre-existing ACC022/ACC040 warnings · all 73 relative links in docs/ resolve). freebuff's handoff listed the docs sync but missed two install surfaces, both now fixed by me: (1) `packages/cli/npm-README.md` — still said "five-phase bootstrap" with the old inspect→verify chain and the old light-init description for `paw init [file]`; now uses the six-stage vocabulary, `npx … init` as the bootstrapper command with `install` as the stable alias, and describes `paw install` as the alias. This is the npm landing page v0.5.2 explicitly wired as an install surface — it must not drift. (2) `docs/.acc-memory.md` — the durable memory still described the five-phase contract and the old `.acc/config/agents/` profile location; corrected in place to the six-stage contract, directory-per-layer layout, and the questionnaire path. Also confirmed harmless: `packages/cli/npm-stage/README.md` still has old text but is git-ignored and regenerated by release.yml at publish time (stale 0.5.0 rehearsal artifact — left alone). Remaining: version bump 0.5.3+ for the release phase exactly as freebuff's entry says (PAW001/PAW008 will demand manifest+changelog sync); nothing else pending.

## [2026-09-25T19:52:41Z] freebuff — decision + implementation: init promoted to the bootstrapper, directory-per-layer layout, questionnaire; UNRELEASED in working tree
For: all

User pinned three decisions for spec §152 and they are now implemented end-to-end (uncommitted): (1) **`paw init` IS the six-stage bootstrapper** (`resolve → acc → shield → proagents → reposell → paw`); `paw install` is a stable ALIAS — `--json` reports `command: "init"`, and the old light-init result shape is gone (init output now has `target` + `stages`). (2) **Directory-per-layer target layout**: `.reposhield/policy.yaml`, `.proagents/{profiles/,crew/,config.yaml}` (profiles moved OUT of `.acc/config/agents/` — ProAgents owns WHO; ACC owns WHAT), `.reposell/distribution.yaml`; crew channel is `.proagents/crew/COMMENTS.md` and the installer NEVER writes `.agents/` in target repos (supersedes the old never-`.agents/` note with a concrete alternative). Old flat files at the legacy paths are preserved, not migrated. (3) **Questionnaire**: empty repo + TTY → `paw init` asks 5 questions inline (product/purpose/technologies/distribution/protection) and applies intent in the same run (pass 2 removes only what pass 1 of the SAME run created — never pre-existing files); headless → `paw init --answers "a" "b" "c" "d" "e"` (bare-flag AND `--answers=a,b,c,d,e` spellings both work; answers take precedence over positional file args — smoke test caught the first-answer-misparsed-as-file bug). Protection answer seeds Repo Shield mode, distribution answer seeds reposell mode. New SDK module `questionnaire.ts` (pure, exported); new error code `QUESTIONNAIRE_EMPTY`. Contract+docs synced: install/install.yaml v2 (stages, not phases), manifest.yaml command `init` + alias, AGENT.md, instructions.md, cli-vocabulary.md, README §152, docs/{bootstrap,cli-reference,getting-started,paw-install-runbook,configuration,index,health-checks}.md, CHANGELOG Unreleased. Gate: lint 0 · TS 0 · **330/330** · build 0 · acc check 0 errors · `paw check` PASS. NOT yet version-bumped or released — next agent: bump 0.5.3+, note PAW001/PAW008 will demand manifest+changelog sync at release time.
