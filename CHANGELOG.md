...
- ProAgents agent environment revised post-convention-first (docs/agent-environment.md pipeline): `proagents.yaml` project description updated from "V1 implementation lands per spec section 134" to the shipped reality (convention-first golden path per spec sections 148–149, V1 landed); `proagent resolve → lock → validate --spec → setup --harness freebuff` re-run — all compiled artifacts (AGENTS.md profile block, .mcp.json, proagents.lock) regenerated BYTE-IDENTICAL, confirming the declaration was structurally in sync; knowledge-file limitation still present in proagent 0.14.0 (release-checklist.json restored manually from the packaged registry); `docs/agent-environment.md` corrected: `paw` is now marked Implemented (was "specification only"), and the honesty section states git guards are declarative in skill text but enforced by the product's guarded git plugin + repo-shield (no longer "once the product implementation lands").
